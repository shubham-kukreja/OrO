...
export class Oracle {
  ...
  private async runTask() {
    ...
    this._taskRunnerTimeout = setTimeout(() => {
      this.runTask();
    }, this._oracleInterval * 1000);
  }

  private async processConfig(config) {
    this._logger.info(
      `Started processing config : ${config.entryPointMichelson}`
    );
    try {
      let dataMichelson = await this.processEndpoint(config);
      dataMichelson = dataMichelson.replace(/'/g, '"');
      console.log("dataMichelson : ", dataMichelson);
      await this._contractHandler.writeToOracleContract(
        config.contractAddress,
        dataMichelson
      );
      this._logger.info(
        `Finished processing config : ${config.entryPointMichelson}`
      );
    } catch (error) {
      this._logger.error(
        `Can not process the config : ${config.entryPointMichelson} : ${error}`
      );
    }
  }
  ...
  private async processEndpoint(config): Promise<string> {
    ...
    config.fields.forEach((field) => {
      switch (field.type) {
        ...
        case "custom":
          ...
          if (field.isString) {
            dataToBeAdded =
              (field.prefix || "") + dataToBeAdded + (field.suffix || "");
            entryPointMichelson = this.formatMichelson(
              entryPointMichelson,
              field.nameInEntryPoint,
              dataToBeAdded
        ...
        case "array":
          entryPointMichelson = this.formatMichelsonArray(
            entryPointMichelson,
            field.nameInEntryPoint,
            response.slice(1, 3)
          );
    ...
    console.log(entryPointMichelson);
  ...
  private formatMichelsonArray(
    entryPointMichelson: string,
    nameInEntryPoint: string,
    array: { player_id: string; rank: string; points: string }[]
  ) {
    const parts = entryPointMichelson.split(nameInEntryPoint);
    let params = parts[0];
    array.forEach((player, index) => {
      params += `Pair ${parseInt(player.player_id)} (Pair ${parseInt(
        player.points
      )} ${parseInt(player.rank)})`;
